...
exports.isActivationLinkExpired = (updatedTime, type = 'link') => {
  ...
  return type === 'link'
    ? linkTime > linkExpiration
    : linkTime > codeExpiration;
};
...
exports.getEmailContent = (id, email, token, role, type = 'signup') => {
  const verificationUrl =
    type === 'signup'
      ? `${appUrl}/${id}/verify/${token}`
      : `${appUrl}${id}/resetPassword/${token}`;
  return {
    to: email,
    subject:
      type === 'signup' ? emailVerificationSubject : resetPasswordSubject,
    content:
      type === 'signup'
        ? getSignupMailContent(verificationUrl, role)
        : getPasswordResetContent(verificationUrl),
  };
};
...
exports.verifyLocation = (longitude, latitude, dataValues) => {
  const minLongitude = parseFloat(dataValues.longitude) - 0.5;
  const maxLongitude = parseFloat(dataValues.longitude) + 0.5;
  const minLatitude = parseFloat(dataValues.latitude) - 0.5;
  const maxLatitude = parseFloat(dataValues.latitude) + 0.5;
...
exports.setProfessionalInitialValues = (
  user,
  phoneInfo,
  bankDetails = {},
  professional = {},
...
exports.getOfferStatusMessage = (status, professionalId = '') => {
  const accepted =
    'Hello, the NMC professional has accepted your shift offer, please confirm your acceptance via the “Approval” button on your NMC Professionals account.';
  const declined =
    'Hello, the requested NMC professional has requested to be excused on this occasion and sends their apologies, sorry.';
  ...
  const rejected =
    'Oops! Offered shift by NMC Professionals has now been filled by someone else. Offers normally work on a first come first serve basis, sorry this time next time will be better.';
